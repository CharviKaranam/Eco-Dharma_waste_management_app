...
import { Leaf, TrendingUp, Sparkles, Quote as QuoteIcon, UploadCloud, Zap, BarChartBig, MapPin, ShoppingBag, Droplets, Users, Gift, Shield, Recycle as RecycleIcon } from 'lucide-react';
...
const QuickActionButtons = ({ userRole }: { userRole: string | null }) => {
  const actions = [
    { label: 'Upload Waste', icon: UploadCloud, href: '/dashboard/upload-waste', roles: ['Household', 'Recycler'] },
    { label: 'Take Action', icon: Zap, href: '/dashboard/take-action', roles: ['Household'] },
    { label: 'Track Waste', icon: BarChartBig, href: '/dashboard/track-waste', roles: ['Household', 'Recycler'] },
    { label: 'EcoMap', icon: MapPin, href: '/dashboard/ecomap', roles: ['Household', 'Recycler', 'NGO', 'Admin'] },
...
export default function DashboardHomePage() {
  const [ecoKarmaPoints, setEcoKarmaPoints] = useState(0);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        <Card className="shadow-md hover:shadow-lg transition-shadow">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-foreground">{ecoKarmaPoints}</div>
